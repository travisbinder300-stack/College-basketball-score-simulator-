...
def example_8_coaching_style_impact():
    """Example: Compare predictions with different coaching styles"""
    print("=" * 60)
    print("Example 8: Coaching Style Impact on Predictions")
    print("=" * 60)
    
    teams = load_team_data('example_teams.json')
    predictor = BasketballPredictor()
    
    # Compare Virginia (slow-tempo) vs North Carolina (fast-paced)
    print("\n1. Slow-Tempo vs Fast-Paced:")
    prediction = predictor.predict_game(teams['Virginia'], teams['North Carolina'])
    print(f"   Virginia (slow-tempo) vs UNC (fast-paced)")
    print(f"   Expected Tempo Impact: Lower scoring game")
    print(f"   Predicted Total: {prediction.total_points:.1f}")
    
    # Compare Duke (offensive) vs Michigan State (defensive)
    print("\n2. Offensive vs Defensive:")
    prediction = predictor.predict_game(teams['Duke'], teams['Michigan State'])
    print(f"   Duke (offensive) vs Michigan State (defensive)")
    print(f"   Style Clash: Offensive efficiency vs defensive focus")
    print(f"   Predicted Spread: Duke {prediction.point_spread:+.1f}")
    
    # Compare Purdue (slow-tempo) vs UCLA (fast-paced)
    print("\n3. Tempo Contrast:")
    prediction = predictor.predict_game(teams['Purdue'], teams['UCLA'])
    print(f"   Purdue (slow-tempo) vs UCLA (fast-paced)")
    print(f"   Tempo styles pulling in opposite directions")
    print(f"   Predicted Total: {prediction.total_points:.1f}")
    print()
...
if __name__ == '__main__':
    ...
    example_6_multiple_games()
    example_7_programmatic_usage()
    example_8_coaching_style_impact()
