...
from mlb_player_props import (  # noqa: E402
    BrewersRoster,
    BREWERS_LINEUP_2026,
    BREWERS_ROTATION_2026,
    BREWERS_BULLPEN_2026,
)
...
class TestBrewersRoster(unittest.TestCase):
    """Tests for BREWERS_LINEUP_2026, BREWERS_ROTATION_2026,
    BREWERS_BULLPEN_2026, and BrewersRoster."""
    ...
    def test_lineup_has_nine_batters(self):
        self.assertEqual(len(BREWERS_LINEUP_2026), 9)

    def test_rotation_has_five_starters(self):
        self.assertEqual(len(BREWERS_ROTATION_2026), 5)

    def test_bullpen_has_six_pitchers(self):
        self.assertEqual(len(BREWERS_BULLPEN_2026), 6)

    def test_lineup_names_in_order(self):
        self.assertEqual([b.name for b in BREWERS_LINEUP_2026], _MIL_LINEUP_NAMES)

    def test_rotation_names_in_order(self):
        self.assertEqual([p.name for p in BREWERS_ROTATION_2026], _MIL_ROTATION_NAMES)

    def test_bullpen_names_in_order(self):
        self.assertEqual([p.name for p in BREWERS_BULLPEN_2026], _MIL_BULLPEN_NAMES)
    ...
    def test_all_batters_pass_validation(self):
        for batter in BREWERS_LINEUP_2026:
            with self.subTest(player=batter.name):
                batter.validate()

    def test_batter_avg_in_range(self):
        for b in BREWERS_LINEUP_2026:
            with self.subTest(player=b.name):
                self.assertGreater(b.avg, 0.180)
                self.assertLess(b.avg, 0.380)
    ...
    def test_contreras_bats_right(self):
        self.assertEqual(BREWERS_LINEUP_2026[0].bats, "R")

    def test_turang_bats_left(self):
        self.assertEqual(BREWERS_LINEUP_2026[2].bats, "L")

    def test_rengifo_bats_switch(self):
        self.assertEqual(BREWERS_LINEUP_2026[3].bats, "S")

    def test_frelick_bats_left(self):
        self.assertEqual(BREWERS_LINEUP_2026[7].bats, "L")

    def test_yelich_bats_left(self):
        self.assertEqual(BREWERS_LINEUP_2026[8].bats, "L")
    ...
    def test_all_rotation_pass_validation(self):
        for p in BREWERS_ROTATION_2026:
            with self.subTest(player=p.name):
                p.validate()

    def test_all_bullpen_pass_validation(self):
        for p in BREWERS_BULLPEN_2026:
            with self.subTest(player=p.name):
                p.validate()
    ...
    def test_woodruff_throws_right(self):
        self.assertEqual(BREWERS_ROTATION_2026[0].throws, "R")

    def test_harrison_throws_left(self):
        self.assertEqual(BREWERS_ROTATION_2026[4].throws, "L")

    def test_koenig_throws_left(self):
        koenig = next(p for p in BREWERS_BULLPEN_2026 if p.name == "Jared Koenig")
        self.assertEqual(koenig.throws, "L")

    def test_ashby_throws_left(self):
        ashby = next(p for p in BREWERS_BULLPEN_2026 if p.name == "Aaron Ashby")
        self.assertEqual(ashby.throws, "L")

    def test_dlhall_throws_left(self):
        dlhall = next(p for p in BREWERS_BULLPEN_2026 if p.name == "DL Hall")
        self.assertEqual(dlhall.throws, "L")

    def test_megill_throws_right(self):
        self.assertEqual(BREWERS_BULLPEN_2026[-1].throws, "R")

    # ------------------------------------------------------------------
    # Relative quality ordering
    # ------------------------------------------------------------------

    def test_woodruff_has_best_rotation_era(self):
        """Brandon Woodruff is modelled as the ace (best ERA) in the rotation."""
        min_era = min(p.era for p in BREWERS_ROTATION_2026)
        self.assertAlmostEqual(BREWERS_ROTATION_2026[0].era, min_era)

    def test_megill_has_best_bullpen_era(self):
        """Trevor Megill (closer) has the best ERA in the bullpen."""
        min_era = min(p.era for p in BREWERS_BULLPEN_2026)
        self.assertAlmostEqual(BREWERS_BULLPEN_2026[-1].era, min_era)

    def test_yelich_has_most_hr_per_600_pa(self):
        max_hr = max(b.hr_per_600_pa for b in BREWERS_LINEUP_2026)
        self.assertAlmostEqual(BREWERS_LINEUP_2026[8].hr_per_600_pa, max_hr)

    def test_yelich_has_highest_power_rating(self):
        max_power = max(b.power_rating for b in BREWERS_LINEUP_2026)
        self.assertAlmostEqual(BREWERS_LINEUP_2026[8].power_rating, max_power)

    def test_chourio_leads_sb(self):
        max_sb = max(b.sb_per_season for b in BREWERS_LINEUP_2026)
        self.assertAlmostEqual(BREWERS_LINEUP_2026[5].sb_per_season, max_sb)

    def test_closer_has_lower_era_than_first_starter(self):
        self.assertLess(BREWERS_BULLPEN_2026[-1].era, BREWERS_ROTATION_2026[0].era)

    def test_frelick_leads_avg(self):
        """Sal Frelick is modelled as the top contact hitter (highest AVG)."""
        max_avg = max(b.avg for b in BREWERS_LINEUP_2026)
        self.assertAlmostEqual(BREWERS_LINEUP_2026[7].avg, max_avg)

    # ------------------------------------------------------------------
    # BrewersRoster dataclass
    # ------------------------------------------------------------------

    def test_default_returns_brewers_roster_instance(self):
        self.assertIsInstance(BrewersRoster.default(), BrewersRoster)

    def test_default_lineup_length(self):
        self.assertEqual(len(BrewersRoster.default().lineup), 9)

    def test_default_rotation_length(self):
        self.assertEqual(len(BrewersRoster.default().rotation), 5)

    def test_default_bullpen_length(self):
        self.assertEqual(len(BrewersRoster.default().bullpen), 6)

    def test_default_lineup_is_copy(self):
        r1 = BrewersRoster.default()
        r2 = BrewersRoster.default()
        r1.lineup.append(r1.lineup[0])
        self.assertEqual(len(r2.lineup), 9)
    ...
    def test_default_lineup_names_match_constants(self):
        ...
        self.assertEqual(
            [b.name for b in roster.lineup],
            [b.name for b in BREWERS_LINEUP_2026],
        )
    ...
    def test_default_rotation_names_match_constants(self):
        ...
        self.assertEqual(
            [p.name for p in roster.rotation],
            [p.name for p in BREWERS_ROTATION_2026],
        )
    ...
    def test_default_bullpen_names_match_constants(self):
        ...
        self.assertEqual(
            [p.name for p in roster.bullpen],
            [p.name for p in BREWERS_BULLPEN_2026],
        )

    def test_default_rotation_ace(self):
        roster = BrewersRoster.default()
        self.assertEqual(roster.rotation[0].name, "Brandon Woodruff")

    def test_default_closer(self):
        roster = BrewersRoster.default()
        self.assertEqual(roster.bullpen[-1].name, "Trevor Megill")

    # ------------------------------------------------------------------
    # Simulator integration
    # ------------------------------------------------------------------

    def test_simulate_pitcher_woodruff_runs(self):
        sim = _make_screener_sim()
        report = sim.simulate_pitcher(BREWERS_ROTATION_2026[0])
        self.assertIsNotNone(report)
        prop_names = [pr.prop_name for pr in report.props]
        self.assertIn("Strikeouts", prop_names)

    def test_simulate_pitcher_megill_runs(self):
        sim = _make_screener_sim()
        report = sim.simulate_pitcher(BREWERS_BULLPEN_2026[-1])
        self.assertIsNotNone(report)
        self.assertTrue(report.props)

    def test_simulate_batter_yelich_runs(self):
        sim = _make_screener_sim()
        report = sim.simulate_batter(BREWERS_LINEUP_2026[8])
        prop_names = [pr.prop_name for pr in report.props]
        self.assertIn("Hits", prop_names)
        self.assertIn("Home Runs", prop_names)

    def test_screen_matchup_brewers_roster(self):
        """screen_matchup with the Brewers lineup against their ace."""
        sim = _make_screener_sim(seed=99)
        screener = UnabatedEdgeScreener(sim, UnabatedClient("key"), min_edge=0.0)
        roster = BrewersRoster.default()
        lines = (
            _make_market_lines("Brandon Woodruff", "strikeouts", 3.5, +350, -600)
            + _make_market_lines("Christian Yelich", "hits", 0.5, +300, -500)
        )
        edges = screener.screen_matchup(
            roster.rotation[0], roster.lineup, market_lines=lines
        )
        player_names = {e.player_name for e in edges}
        self.assertIn("Brandon Woodruff", player_names)
        self.assertIn("Christian Yelich", player_names)


from mlb_player_props import (  # noqa: E402
    PiratesRoster,
    PIRATES_LINEUP_2026,
    PIRATES_ROTATION_2026,
    PIRATES_BULLPEN_2026,
)
...
class TestPiratesRoster(unittest.TestCase):
    """Tests for PIRATES_LINEUP_2026, PIRATES_ROTATION_2026,
    PIRATES_BULLPEN_2026, and PiratesRoster."""
    ...
    def test_lineup_has_nine_batters(self):
        self.assertEqual(len(PIRATES_LINEUP_2026), 9)

    def test_rotation_has_five_starters(self):
        self.assertEqual(len(PIRATES_ROTATION_2026), 5)

    def test_bullpen_has_six_pitchers(self):
        self.assertEqual(len(PIRATES_BULLPEN_2026), 6)

    def test_lineup_names_in_order(self):
        self.assertEqual([b.name for b in PIRATES_LINEUP_2026], _PIT_LINEUP_NAMES)

    def test_rotation_names_in_order(self):
        self.assertEqual([p.name for p in PIRATES_ROTATION_2026], _PIT_ROTATION_NAMES)

    def test_bullpen_names_in_order(self):
        self.assertEqual([p.name for p in PIRATES_BULLPEN_2026], _PIT_BULLPEN_NAMES)
    ...
    def test_all_batters_pass_validation(self):
        for batter in PIRATES_LINEUP_2026:
            with self.subTest(player=batter.name):
                batter.validate()

    def test_batter_avg_in_range(self):
        for b in PIRATES_LINEUP_2026:
            with self.subTest(player=b.name):
                self.assertGreater(b.avg, 0.180)
                self.assertLess(b.avg, 0.380)
    ...
    def test_davis_bats_right(self):
        self.assertEqual(PIRATES_LINEUP_2026[0].bats, "R")

    def test_horwitz_bats_left(self):
        self.assertEqual(PIRATES_LINEUP_2026[1].bats, "L")

    def test_lowe_bats_left(self):
        self.assertEqual(PIRATES_LINEUP_2026[2].bats, "L")

    def test_reynolds_bats_switch(self):
        self.assertEqual(PIRATES_LINEUP_2026[5].bats, "S")

    def test_cruz_bats_left(self):
        self.assertEqual(PIRATES_LINEUP_2026[6].bats, "L")

    def test_ozuna_bats_right(self):
        self.assertEqual(PIRATES_LINEUP_2026[8].bats, "R")
    ...
    def test_all_rotation_pass_validation(self):
        for p in PIRATES_ROTATION_2026:
            with self.subTest(player=p.name):
                p.validate()

    def test_all_bullpen_pass_validation(self):
        for p in PIRATES_BULLPEN_2026:
            with self.subTest(player=p.name):
                p.validate()
    ...
    def test_skenes_throws_right(self):
        self.assertEqual(PIRATES_ROTATION_2026[0].throws, "R")

    def test_mlodzinski_throws_right(self):
        self.assertEqual(PIRATES_ROTATION_2026[4].throws, "R")

    def test_soto_throws_left(self):
        soto = next(p for p in PIRATES_BULLPEN_2026 if p.name == "Gregory Soto")
        self.assertEqual(soto.throws, "L")

    def test_montgomery_throws_left(self):
        montgomery = next(p for p in PIRATES_BULLPEN_2026 if p.name == "Mason Montgomery")
        self.assertEqual(montgomery.throws, "L")

    def test_santana_throws_right(self):
        self.assertEqual(PIRATES_BULLPEN_2026[-1].throws, "R")

    # ------------------------------------------------------------------
    # Relative quality ordering
    # ------------------------------------------------------------------

    def test_skenes_has_best_rotation_era(self):
        """Paul Skenes is modelled as the ace (best ERA) in the rotation."""
        min_era = min(p.era for p in PIRATES_ROTATION_2026)
        self.assertAlmostEqual(PIRATES_ROTATION_2026[0].era, min_era)

    def test_santana_has_best_bullpen_era(self):
        """Dennis Santana (closer) has the best ERA in the bullpen."""
        min_era = min(p.era for p in PIRATES_BULLPEN_2026)
        self.assertAlmostEqual(PIRATES_BULLPEN_2026[-1].era, min_era)

    def test_ozuna_has_most_hr_per_600_pa(self):
        max_hr = max(b.hr_per_600_pa for b in PIRATES_LINEUP_2026)
        self.assertAlmostEqual(PIRATES_LINEUP_2026[8].hr_per_600_pa, max_hr)

    def test_ozuna_has_highest_power_rating(self):
        max_power = max(b.power_rating for b in PIRATES_LINEUP_2026)
        self.assertAlmostEqual(PIRATES_LINEUP_2026[8].power_rating, max_power)

    def test_cruz_leads_sb(self):
        max_sb = max(b.sb_per_season for b in PIRATES_LINEUP_2026)
        self.assertAlmostEqual(PIRATES_LINEUP_2026[6].sb_per_season, max_sb)

    def test_closer_has_lower_era_than_first_starter(self):
        self.assertLess(PIRATES_BULLPEN_2026[-1].era, PIRATES_ROTATION_2026[0].era)

    # ------------------------------------------------------------------
    # PiratesRoster dataclass
    # ------------------------------------------------------------------

    def test_default_returns_pirates_roster_instance(self):
        self.assertIsInstance(PiratesRoster.default(), PiratesRoster)

    def test_default_lineup_length(self):
        self.assertEqual(len(PiratesRoster.default().lineup), 9)

    def test_default_rotation_length(self):
        self.assertEqual(len(PiratesRoster.default().rotation), 5)

    def test_default_bullpen_length(self):
        self.assertEqual(len(PiratesRoster.default().bullpen), 6)

    def test_default_lineup_is_copy(self):
        r1 = PiratesRoster.default()
        r2 = PiratesRoster.default()
        r1.lineup.append(r1.lineup[0])
        self.assertEqual(len(r2.lineup), 9)
    ...
    def test_default_lineup_names_match_constants(self):
        ...
        self.assertEqual(
            [b.name for b in roster.lineup],
            [b.name for b in PIRATES_LINEUP_2026],
        )
    ...
    def test_default_rotation_names_match_constants(self):
        ...
        self.assertEqual(
            [p.name for p in roster.rotation],
            [p.name for p in PIRATES_ROTATION_2026],
        )
    ...
    def test_default_bullpen_names_match_constants(self):
        ...
        self.assertEqual(
            [p.name for p in roster.bullpen],
            [p.name for p in PIRATES_BULLPEN_2026],
        )

    def test_default_rotation_ace(self):
        roster = PiratesRoster.default()
        self.assertEqual(roster.rotation[0].name, "Paul Skenes")

    def test_default_closer(self):
        roster = PiratesRoster.default()
        self.assertEqual(roster.bullpen[-1].name, "Dennis Santana")

    # ------------------------------------------------------------------
    # Simulator integration
    # ------------------------------------------------------------------

    def test_simulate_pitcher_skenes_runs(self):
        sim = _make_screener_sim()
        report = sim.simulate_pitcher(PIRATES_ROTATION_2026[0])
        self.assertIsNotNone(report)
        prop_names = [pr.prop_name for pr in report.props]
        self.assertIn("Strikeouts", prop_names)

    def test_simulate_pitcher_santana_runs(self):
        sim = _make_screener_sim()
        report = sim.simulate_pitcher(PIRATES_BULLPEN_2026[-1])
        self.assertIsNotNone(report)
        self.assertTrue(report.props)

    def test_simulate_batter_ozuna_runs(self):
        sim = _make_screener_sim()
        report = sim.simulate_batter(PIRATES_LINEUP_2026[8])
        prop_names = [pr.prop_name for pr in report.props]
        self.assertIn("Hits", prop_names)
        self.assertIn("Home Runs", prop_names)

    def test_screen_matchup_pirates_roster(self):
        """screen_matchup with the Pirates lineup against their ace."""
        sim = _make_screener_sim(seed=99)
        screener = UnabatedEdgeScreener(sim, UnabatedClient("key"), min_edge=0.0)
        roster = PiratesRoster.default()
        lines = (
            _make_market_lines("Paul Skenes", "strikeouts", 3.5, +350, -600)
            + _make_market_lines("Marcell Ozuna", "hits", 0.5, +300, -500)
        )
        edges = screener.screen_matchup(
            roster.rotation[0], roster.lineup, market_lines=lines
        )
        player_names = {e.player_name for e in edges}
        self.assertIn("Paul Skenes", player_names)
...
class TestBravesRoster(unittest.TestCase):
    """Tests for BRAVES_LINEUP_2026, BRAVES_ROTATION_2026,
    BRAVES_BULLPEN_2026, and BravesRoster."""
    ...
    def test_lineup_has_nine_batters(self):
        self.assertEqual(len(BRAVES_LINEUP_2026), 9)

    def test_rotation_has_five_starters(self):
        self.assertEqual(len(BRAVES_ROTATION_2026), 5)

    def test_bullpen_has_six_pitchers(self):
        self.assertEqual(len(BRAVES_BULLPEN_2026), 6)

    def test_lineup_names_in_order(self):
        self.assertEqual([b.name for b in BRAVES_LINEUP_2026], _ATL_LINEUP_NAMES)

    def test_rotation_names_in_order(self):
        self.assertEqual([p.name for p in BRAVES_ROTATION_2026], _ATL_ROTATION_NAMES)

    def test_bullpen_names_in_order(self):
        self.assertEqual([p.name for p in BRAVES_BULLPEN_2026], _ATL_BULLPEN_NAMES)
    ...
    def test_all_batters_pass_validation(self):
        for batter in BRAVES_LINEUP_2026:
            with self.subTest(player=batter.name):
                batter.validate()

    def test_batter_avg_in_range(self):
        for b in BRAVES_LINEUP_2026:
            with self.subTest(player=b.name):
                self.assertGreater(b.avg, 0.180)
                self.assertLess(b.avg, 0.380)
    ...
    def test_baldwin_bats_right(self):
        self.assertEqual(BRAVES_LINEUP_2026[0].bats, "R")

    def test_olson_bats_left(self):
        self.assertEqual(BRAVES_LINEUP_2026[1].bats, "L")

    def test_albies_bats_switch(self):
        self.assertEqual(BRAVES_LINEUP_2026[2].bats, "S")

    def test_riley_bats_right(self):
        self.assertEqual(BRAVES_LINEUP_2026[3].bats, "R")

    def test_yastrzemski_bats_left(self):
        self.assertEqual(BRAVES_LINEUP_2026[5].bats, "L")

    def test_harris_bats_left(self):
        self.assertEqual(BRAVES_LINEUP_2026[6].bats, "L")

    def test_acuna_bats_right(self):
        self.assertEqual(BRAVES_LINEUP_2026[7].bats, "R")
    ...
    def test_all_rotation_pass_validation(self):
        for p in BRAVES_ROTATION_2026:
            with self.subTest(player=p.name):
                p.validate()

    def test_all_bullpen_pass_validation(self):
        for p in BRAVES_BULLPEN_2026:
            with self.subTest(player=p.name):
                p.validate()
    ...
    def test_sale_throws_left(self):
        self.assertEqual(BRAVES_ROTATION_2026[0].throws, "L")

    def test_strider_throws_right(self):
        self.assertEqual(BRAVES_ROTATION_2026[1].throws, "R")

    def test_lee_throws_left(self):
        lee = next(p for p in BRAVES_BULLPEN_2026 if p.name == "Dylan Lee")
        self.assertEqual(lee.throws, "L")

    def test_bummer_throws_left(self):
        bummer = next(p for p in BRAVES_BULLPEN_2026 if p.name == "Aaron Bummer")
        self.assertEqual(bummer.throws, "L")

    def test_iglesias_throws_right(self):
        self.assertEqual(BRAVES_BULLPEN_2026[-1].throws, "R")

    # ------------------------------------------------------------------
    # Relative quality ordering
    # ------------------------------------------------------------------

    def test_sale_has_best_rotation_era(self):
        """Chris Sale is modelled as the ace (best ERA) in the rotation."""
        min_era = min(p.era for p in BRAVES_ROTATION_2026)
        self.assertAlmostEqual(BRAVES_ROTATION_2026[0].era, min_era)

    def test_iglesias_has_best_bullpen_era(self):
        """Raisel Iglesias (closer) has the best ERA in the bullpen."""
        min_era = min(p.era for p in BRAVES_BULLPEN_2026)
        self.assertAlmostEqual(BRAVES_BULLPEN_2026[-1].era, min_era)

    def test_olson_has_most_hr_per_600_pa(self):
        max_hr = max(b.hr_per_600_pa for b in BRAVES_LINEUP_2026)
        self.assertAlmostEqual(BRAVES_LINEUP_2026[1].hr_per_600_pa, max_hr)

    def test_olson_has_highest_power_rating(self):
        max_power = max(b.power_rating for b in BRAVES_LINEUP_2026)
        self.assertAlmostEqual(BRAVES_LINEUP_2026[1].power_rating, max_power)

    def test_acuna_leads_sb(self):
        max_sb = max(b.sb_per_season for b in BRAVES_LINEUP_2026)
        self.assertAlmostEqual(BRAVES_LINEUP_2026[7].sb_per_season, max_sb)

    def test_closer_has_lower_era_than_first_starter(self):
        self.assertLess(BRAVES_BULLPEN_2026[-1].era, BRAVES_ROTATION_2026[0].era)

    # ------------------------------------------------------------------
    # BravesRoster dataclass
    # ------------------------------------------------------------------

    def test_default_returns_braves_roster_instance(self):
        self.assertIsInstance(BravesRoster.default(), BravesRoster)

    def test_default_lineup_length(self):
        self.assertEqual(len(BravesRoster.default().lineup), 9)

    def test_default_rotation_length(self):
        self.assertEqual(len(BravesRoster.default().rotation), 5)

    def test_default_bullpen_length(self):
        self.assertEqual(len(BravesRoster.default().bullpen), 6)

    def test_default_lineup_is_copy(self):
        r1 = BravesRoster.default()
        r2 = BravesRoster.default()
        r1.lineup.append(r1.lineup[0])
        self.assertEqual(len(r2.lineup), 9)
...
from mlb_player_props import (  # noqa: E402
    PhilliesRoster,
    PHILLIES_LINEUP_2026,
    PHILLIES_ROTATION_2026,
    PHILLIES_BULLPEN_2026,
)
...
class TestPhilliesRoster(unittest.TestCase):
    """Tests for PHILLIES_LINEUP_2026, PHILLIES_ROTATION_2026,
    PHILLIES_BULLPEN_2026, and PhilliesRoster."""
    ...
    def test_lineup_length(self):
        self.assertEqual(len(PHILLIES_LINEUP_2026), 9)

    def test_rotation_length(self):
        self.assertEqual(len(PHILLIES_ROTATION_2026), 5)

    def test_bullpen_length(self):
        self.assertEqual(len(PHILLIES_BULLPEN_2026), 6)

    def test_lineup_names_in_order(self):
        self.assertEqual([b.name for b in PHILLIES_LINEUP_2026], _PHI_LINEUP_NAMES)

    def test_rotation_names_in_order(self):
        self.assertEqual([p.name for p in PHILLIES_ROTATION_2026], _PHI_ROTATION_NAMES)

    def test_bullpen_names_in_order(self):
        self.assertEqual([p.name for p in PHILLIES_BULLPEN_2026], _PHI_BULLPEN_NAMES)
    ...
    def test_all_batters_pass_validation(self):
        for batter in PHILLIES_LINEUP_2026:
            with self.subTest(player=batter.name):
                batter.validate()

    def test_batter_avg_in_range(self):
        for b in PHILLIES_LINEUP_2026:
            with self.subTest(player=b.name):
                self.assertGreater(b.avg, 0.180)
                self.assertLess(b.avg, 0.380)
    ...
    def test_realmuto_bats_right(self):
        self.assertEqual(PHILLIES_LINEUP_2026[0].bats, "R")

    def test_harper_bats_left(self):
        self.assertEqual(PHILLIES_LINEUP_2026[1].bats, "L")

    def test_stott_bats_left(self):
        self.assertEqual(PHILLIES_LINEUP_2026[2].bats, "L")

    def test_bohm_bats_right(self):
        self.assertEqual(PHILLIES_LINEUP_2026[3].bats, "R")

    def test_turner_bats_right(self):
        self.assertEqual(PHILLIES_LINEUP_2026[4].bats, "R")

    def test_marsh_bats_left(self):
        self.assertEqual(PHILLIES_LINEUP_2026[5].bats, "L")

    def test_crawford_bats_left(self):
        self.assertEqual(PHILLIES_LINEUP_2026[6].bats, "L")

    def test_garcia_bats_right(self):
        self.assertEqual(PHILLIES_LINEUP_2026[7].bats, "R")

    def test_schwarber_bats_left(self):
        self.assertEqual(PHILLIES_LINEUP_2026[8].bats, "L")
    ...
    def test_all_rotation_pass_validation(self):
        for p in PHILLIES_ROTATION_2026:
            with self.subTest(player=p.name):
                p.validate()

    def test_all_bullpen_pass_validation(self):
        for p in PHILLIES_BULLPEN_2026:
            with self.subTest(player=p.name):
                p.validate()
    ...
    def test_wheeler_throws_right(self):
        self.assertEqual(PHILLIES_ROTATION_2026[0].throws, "R")

    def test_sanchez_throws_left(self):
        self.assertEqual(PHILLIES_ROTATION_2026[1].throws, "L")

    def test_luzardo_throws_left(self):
        self.assertEqual(PHILLIES_ROTATION_2026[2].throws, "L")

    def test_alvarado_throws_left(self):
        alvarado = next(p for p in PHILLIES_BULLPEN_2026 if p.name == "Jose Alvarado")
        self.assertEqual(alvarado.throws, "L")

    def test_duran_throws_right(self):
        self.assertEqual(PHILLIES_BULLPEN_2026[-1].throws, "R")

    # ------------------------------------------------------------------
    # Relative quality ordering
    # ------------------------------------------------------------------

    def test_wheeler_has_best_rotation_era(self):
        """Zack Wheeler is modelled as the ace (best ERA) in the rotation."""
        min_era = min(p.era for p in PHILLIES_ROTATION_2026)
        self.assertAlmostEqual(PHILLIES_ROTATION_2026[0].era, min_era)

    def test_duran_has_best_bullpen_era(self):
        """Jhoan Duran (closer) has the best ERA in the bullpen."""
        min_era = min(p.era for p in PHILLIES_BULLPEN_2026)
        self.assertAlmostEqual(PHILLIES_BULLPEN_2026[-1].era, min_era)

    def test_schwarber_has_most_hr_per_600_pa(self):
        max_hr = max(b.hr_per_600_pa for b in PHILLIES_LINEUP_2026)
        self.assertAlmostEqual(PHILLIES_LINEUP_2026[8].hr_per_600_pa, max_hr)

    def test_schwarber_has_highest_power_rating(self):
        max_power = max(b.power_rating for b in PHILLIES_LINEUP_2026)
        self.assertAlmostEqual(PHILLIES_LINEUP_2026[8].power_rating, max_power)

    def test_turner_leads_sb(self):
        max_sb = max(b.sb_per_season for b in PHILLIES_LINEUP_2026)
        self.assertAlmostEqual(PHILLIES_LINEUP_2026[4].sb_per_season, max_sb)

    def test_closer_has_lower_era_than_first_starter(self):
        self.assertLess(PHILLIES_BULLPEN_2026[-1].era, PHILLIES_ROTATION_2026[0].era)

    # ------------------------------------------------------------------
    # PhilliesRoster dataclass
    # ------------------------------------------------------------------

    def test_default_returns_phillies_roster_instance(self):
        self.assertIsInstance(PhilliesRoster.default(), PhilliesRoster)

    def test_default_lineup_length(self):
        self.assertEqual(len(PhilliesRoster.default().lineup), 9)

    def test_default_rotation_length(self):
        self.assertEqual(len(PhilliesRoster.default().rotation), 5)

    def test_default_bullpen_length(self):
        self.assertEqual(len(PhilliesRoster.default().bullpen), 6)

    def test_default_lineup_is_copy(self):
        r1 = PhilliesRoster.default()
        r2 = PhilliesRoster.default()
        r1.lineup.append(r1.lineup[0])
        self.assertEqual(len(r2.lineup), 9)
...
from mlb_player_props import (  # noqa: E402
    NationalsRoster,
    NATIONALS_LINEUP_2026,
    NATIONALS_ROTATION_2026,
    NATIONALS_BULLPEN_2026,
)
...
class TestNationalsRoster(unittest.TestCase):
    """Tests for NATIONALS_LINEUP_2026, NATIONALS_ROTATION_2026,
    NATIONALS_BULLPEN_2026, and NationalsRoster."""
    ...
    def test_lineup_length(self):
        self.assertEqual(len(NATIONALS_LINEUP_2026), 9)

    def test_rotation_length(self):
        self.assertEqual(len(NATIONALS_ROTATION_2026), 5)

    def test_bullpen_length(self):
        self.assertEqual(len(NATIONALS_BULLPEN_2026), 6)

    def test_lineup_names_in_order(self):
        self.assertEqual([b.name for b in NATIONALS_LINEUP_2026], _WSH_LINEUP_NAMES)

    def test_rotation_names_in_order(self):
        self.assertEqual([p.name for p in NATIONALS_ROTATION_2026], _WSH_ROTATION_NAMES)

    def test_bullpen_names_in_order(self):
        self.assertEqual([p.name for p in NATIONALS_BULLPEN_2026], _WSH_BULLPEN_NAMES)
    ...
    def test_all_batters_pass_validation(self):
        for batter in NATIONALS_LINEUP_2026:
            with self.subTest(player=batter.name):
                batter.validate()

    def test_batter_avg_in_range(self):
        for b in NATIONALS_LINEUP_2026:
            with self.subTest(player=b.name):
                self.assertGreater(b.avg, 0.180)
                self.assertLess(b.avg, 0.380)
    ...
    def test_ruiz_bats_switch(self):
        self.assertEqual(NATIONALS_LINEUP_2026[0].bats, "S")

    def test_chaparro_bats_right(self):
        self.assertEqual(NATIONALS_LINEUP_2026[1].bats, "R")

    def test_lgarcia_bats_right(self):
        self.assertEqual(NATIONALS_LINEUP_2026[2].bats, "R")

    def test_house_bats_right(self):
        self.assertEqual(NATIONALS_LINEUP_2026[3].bats, "R")

    def test_abrams_bats_left(self):
        self.assertEqual(NATIONALS_LINEUP_2026[4].bats, "L")

    def test_wood_bats_left(self):
        self.assertEqual(NATIONALS_LINEUP_2026[5].bats, "L")

    def test_young_bats_right(self):
        self.assertEqual(NATIONALS_LINEUP_2026[6].bats, "R")

    def test_wiemer_bats_right(self):
        self.assertEqual(NATIONALS_LINEUP_2026[7].bats, "R")

    def test_lile_bats_left(self):
        self.assertEqual(NATIONALS_LINEUP_2026[8].bats, "L")
    ...
    def test_all_rotation_pass_validation(self):
        for p in NATIONALS_ROTATION_2026:
            with self.subTest(player=p.name):
                p.validate()

    def test_all_bullpen_pass_validation(self):
        for p in NATIONALS_BULLPEN_2026:
            with self.subTest(player=p.name):
                p.validate()
    ...
    def test_cavalli_throws_right(self):
        self.assertEqual(NATIONALS_ROTATION_2026[0].throws, "R")

    def test_griffin_throws_left(self):
        self.assertEqual(NATIONALS_ROTATION_2026[2].throws, "L")

    def test_perez_throws_left(self):
        perez = next(p for p in NATIONALS_BULLPEN_2026 if p.name == "Cionel Perez")
        self.assertEqual(perez.throws, "L")

    def test_beeter_throws_right(self):
        self.assertEqual(NATIONALS_BULLPEN_2026[-1].throws, "R")

    # ------------------------------------------------------------------
    # Relative quality ordering
    # ------------------------------------------------------------------

    def test_cavalli_has_best_rotation_era(self):
        """Cade Cavalli is modelled as the ace (best ERA) in the rotation."""
        min_era = min(p.era for p in NATIONALS_ROTATION_2026)
        self.assertAlmostEqual(NATIONALS_ROTATION_2026[0].era, min_era)

    def test_beeter_has_best_bullpen_era(self):
        """Clayton Beeter (closer) has the best ERA in the bullpen."""
        min_era = min(p.era for p in NATIONALS_BULLPEN_2026)
        self.assertAlmostEqual(NATIONALS_BULLPEN_2026[-1].era, min_era)

    def test_wood_has_most_hr_per_600_pa(self):
        max_hr = max(b.hr_per_600_pa for b in NATIONALS_LINEUP_2026)
        self.assertAlmostEqual(NATIONALS_LINEUP_2026[5].hr_per_600_pa, max_hr)

    def test_wood_has_highest_power_rating(self):
        max_power = max(b.power_rating for b in NATIONALS_LINEUP_2026)
        self.assertAlmostEqual(NATIONALS_LINEUP_2026[5].power_rating, max_power)

    def test_abrams_leads_sb(self):
        max_sb = max(b.sb_per_season for b in NATIONALS_LINEUP_2026)
        self.assertAlmostEqual(NATIONALS_LINEUP_2026[4].sb_per_season, max_sb)

    def test_closer_has_lower_era_than_first_starter(self):
        self.assertLess(NATIONALS_BULLPEN_2026[-1].era, NATIONALS_ROTATION_2026[0].era)

    # ------------------------------------------------------------------
    # NationalsRoster dataclass
    # ------------------------------------------------------------------

    def test_default_returns_nationals_roster_instance(self):
        self.assertIsInstance(NationalsRoster.default(), NationalsRoster)

    def test_default_lineup_length(self):
        self.assertEqual(len(NationalsRoster.default().lineup), 9)

    def test_default_rotation_length(self):
        self.assertEqual(len(NationalsRoster.default().rotation), 5)

    def test_default_bullpen_length(self):
        self.assertEqual(len(NationalsRoster.default().bullpen), 6)

    def test_default_lineup_is_copy(self):
        r1 = NationalsRoster.default()
        r2 = NationalsRoster.default()
        r1.lineup.append(r1.lineup[0])
        self.assertEqual(len(r2.lineup), 9)
